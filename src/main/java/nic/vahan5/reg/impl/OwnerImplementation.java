...
public class OwnerImplementation {
    ...
    @Autowired
    OtherStateVehImpl otherStateVehImpl;
    @Autowired
    ...
    CdImpl cdImpl;
    @Autowired
    PendencyBankDetailImplementation pendencyBankImpl;
    @Autowired
    ...
    @Autowired
    RetroFittingDetailsImplementation retroFitImpl;
    
    @Autowired
    ImportedVehicleImplementation importedVehicleImpl;
    @Autowired
       ...
       // TransactionManager tmgr = null;
        ...
        try {
         ...
         //   tmgr = new TransactionManager("getValidateVtOwnerTempDobj");
            ...
            ps.setString(1, parameterValue);
            rs = tmg.fetchDetachedRowSet_No_release(); //state_cd_to, off_cd_to
            if (rs.next() && rs.getBoolean("v4_started")) {
                stateTo = rs.getString("state_cd_to").trim();
                offTo = rs.getInt("off_cd_to");
                purpose = rs.getString("purpose");
                ...
                if (stateTo.equals(userStateCode) && (offTo == offCode)
                    ...
                    //RetroFitting Details
                    RetroFittingDetailsDobj retroDobj = retroFitImpl.setCngDetails_db_to_dobj_VT(owner_dobj.getRegn_no(), rs.getString("state_cd"), rs.getInt("off_cd"), cnginfo_from_cngmaker);
                    owner_dobj.setCng_dobj(retroDobj);
...
/*
          ...
          //  tmgr = new TransactionManager("Owner_Impl");
            ...
            if (TableConstants.VM_TRANSACTION_MAST_NEW_VEHICLE == pur_cd
                ...
                if (appl_no != null && !appl_no.equalsIgnoreCase("")) {
                    ...
                    owner_dobj = fillFrom_Va_OwnerDobj(rs);
                    if (owner_dobj != null) {
                        ps = tmg.prepareStatement("select * from va_owner_temp where appl_no=? ");
                        ps.setString(1, parameterValue);
                        rs = tmg.fetchDetachedRowSet_No_release();
                        Owner_temp_dobj temp = fillFrom_va_owner_temp(rs);
                        if (temp != null) {
    ...
    public  void validatePurchaseAs(Owner_dobj dobj, String regnType, int purCd) throws VahanException {
        ...
        if (rs.next()) {
            ...
            //    tmgr = new TransactionManager("Owner_Id");
                ...
                String sqlOwnerId = "SELECT * FROM " + TableList.VT_OWNER_IDENTIFICATION
                        + " where regn_no =? and state_cd = ? and off_cd = ? ";
                PreparedStatement ps = tmgr.prepareStatement(sqlOwnerId);
                ps.setString(1, dobj.getRegn_no());
                ps.setString(2, dobj.getState_cd());
                ps.setInt(3, dobj.getOff_cd());
                rsOwnerId = tmg.fetchDetachedRowSet_No_release();
                ...
                FitnessImplementation fitImpl = new FitnessImplementation();
                ReflectiveTapeDobj refDobj = fitImpl.getVtReflectiveTapeDobj(dobj.getRegn_no(), dobj.getState_cd(), dobj.getOff_cd(), tmgr);
          ...
          //  TransactionManager tmgrOwner = null;
            try {
               // tmgrOwner = new TransactionManager("Owner_Id");
                ...
                String sqlOwnerId = "SELECT * FROM " + TableList.VT_OWNER_IDENTIFICATION
                        + " where regn_no =? and state_cd = ? and off_cd = ? ";
                PreparedStatement ps = tmg.prepareStatement(sqlOwnerId);
                ps.setString(1, dobj.getRegn_no());
                ps.setString(2, stateCode);
                ps.setInt(3, selectedOffCode);
                rsOwnerId = tmg.fetchDetachedRowSet_No_release();
                ...
                ReflectiveTapeDobj refDobj = new FitnessImplementation().getVtReflectiveTapeDobj(dobj.getRegn_no(), stateCode, selectedOffCode, tmgr);
                dobj.setReflectiveTapeDobj(refDobj);
              ...
              //  tmgrOwner = new TransactionManager("Owner_Id");
               ...
               // FitnessImplementation fitImpl = new FitnessImplementation();
                ...
                if ("DL".equals(owner_dobj.getState_cd()) && owner_dobj.getVh_class() == TableConstants.ERICKSHAW_VCH_CLASS) {
                    PendencyBankDobj dobj = pendencyBankImpl.getBankSubsidyData(rsOwner.getString("appl_no"), rsOwner.getString("state_cd"), rsOwner.getInt("off_cd"));
                    if (dobj != null) {
                        owner_dobj.getOwner_identity().setAadhar_no(dobj.getAadharNo());
                    }
                }
                // auction Details
                AuctionDobj auctionDobj = auctionImpl.getDetailsFromVtAuction(owner_dobj.getChasi_no().trim().toUpperCase(), null);
...
//            boolean allow_fitness_all_rto = allowFitnessAllRto;
            ...
            if (rs.next()) {
                ...
                String sqlOwnerId = "SELECT * FROM " + TableList.VT_OWNER_IDENTIFICATION
                        + " WHERE regn_no =? and state_cd = ? and off_cd  = ?";
                ps = tmg.prepareStatement(sqlOwnerId);
                ps.setString(1, dobj.getRegn_no());
                ps.setString(2, state_cd);
//                if (Util.getSelectedSeat() != null) {
//                    ps.setInt(3, Util.getSelectedSeat().getOff_cd());
          ...
          //  tmgr = new TransactionManagerReadOnly("getOwnerDetails");
            ...
            while (rs.next()) {
                ...
                if (stateCd != null && stateCd.equalsIgnoreCase("OR") && TableConstants.GCW_VEH_CLASS.contains(vh_classCase)) {
                    ...
                    ps = tmg.prepareStatement(sqlGcw);
                    ...
                    RowSet rset = tmg.fetchDetachedRowSet_No_release();
                    int ldwt = 0;
                    while (rset.next()) {
                ...
                RowSet rs1 = tmg.fetchDetachedRowSet_No_release();
                ...
                ps = tmg.prepareStatement(lastRcptdt);
                ...
                RowSet rs2 = tmg.fetchDetachedRowSet_No_release();
                if (rs2.next()) {
                ...
                String sqlNoOftyres = "SELECT sum(COALESCE(f_axle_tyre,0) + COALESCE(r_axle_tyre,0) + COALESCE(o_axle_tyre,0) + COALESCE(t_axle_tyre,0)) as numberoftyre FROM " + TableList.VT_AXLE
                        + " WHERE regn_no =? and state_cd = ? and off_cd = ? ";
                ps = tmg.prepareStatement(sqlNoOftyres);
                ps.setString(1, dobj.getRegn_no());
                ps.setString(2, dobj.getState_cd());
                ps.setInt(3, dobj.getOff_cd());
                RowSet rs3 = tmg.fetchDetachedRowSet_No_release();
                if (rs3.next()) {
                    dobj.setNumberOfTyres(rs3.getInt("numberoftyre"));
                }
                trailer_dobjList = Trailer_Impl.set_trailer_dobjList(tmg, null, regnNo, 0);
...
/*
   ...
   */
    private String getLinkedVehDetails(String applNo, TransactionManagerOne tmgr) throws SQLException {
        String linkedRegnNo = "";
